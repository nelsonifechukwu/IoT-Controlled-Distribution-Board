...
#design the web_api
@app.route('/update/key=<api_key>/voltage=<v>/c1=<int:c1>/c2=<int:c2>/c3=<int:c3>/c4=<int:c4>/c5=<int:c5>/c6=<int:c6>/c7=<int:c7>/c8=<int:c8>', methods=['GET'])
def update(api_key, c1, c2, c3, c4, c5, c6, c7, c8, v):
    if (api_key == API):
        insert_distribution_data(c1, c2, c3, c4, c5, c6, c7, c8, v)
...
def update_state(id):
    ...
    return str(newstate)
    #all_state = int(state), newstate
    #return str(all_state) #Response(200)
...
def delete():
    if request.method == 'POST':
        delete_dist_data()
        insert_distribution_data(0,0,0,0,0,0,0,0,0)
        return redirect(url_for('plot'))
